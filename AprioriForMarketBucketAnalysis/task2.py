...
if __name__ == '__main__':
    ...
    # Initial Parameter
    filter_threshold = int(sys.argv[1])     # Filter user_id with review under this number
    support = int(sys.argv[2])              # Support for frequent item set
    ...
    rdd = sc.textFile(input_file).map(lambda line: tuple(line.split(',')))

    # Remove header in rdd and duplicate pairs
    header = rdd.first()
    rdd = rdd.filter(lambda line: line != header).distinct()

    # Filter for qualified users
    qualified_user = rdd.groupByKey() \
        .map(lambda line: tuple(line[1])) \
        .filter(lambda line: len(line) > filter_threshold).persist()

    # Apply SON algorithm
    baskets = qualified_user.collect()      # Whole data set
    p = qualified_user.getNumPartitions()   # Number of partitions
    ...
    while True:
        ...
        if candidates_list:
            candidate = qualified_user.mapPartitions(lambda line: pcy(line, p, support, k, candidates_list[-1])).distinct()
        else:
            candidate = qualified_user.mapPartitions(lambda line: pcy(line, p, support, k)).distinct().map(lambda line: [line])
        # Get candidate list
        c = candidate.collect()
        ...
        # Get actual frequent item set
        f = frequent.collect()

        k += 1                          # Update size of k by 1
        candidates_list.append(c)       # Add candidate of size at k to candidate_list
        frequent_list.append(f)         # Add actural frequent item set of size k to frequent_list
